...
export default function Profile() {
  ...
  useEffect(() => {
    if (userId) drivers.getProfile(userId).then(setProfile).finally(() => setLoading(false));
  }, [userId]);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white border rounded-xl p-6 flex flex-col gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{profile.vehicle_type} · {profile.license_plate}</p>
        </div>
