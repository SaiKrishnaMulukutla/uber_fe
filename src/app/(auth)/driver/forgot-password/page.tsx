...
export default function DriverForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await driverForgotPassword(email)
      toast.success('OTP sent to your email')
      router.push(`/driver/reset-password?email=${encodeURIComponent(email)}`)
    } catch (err) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-black to-zinc-900 px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="mt-5 text-center text-sm">
          <Link href="/driver/login" className="text-zinc-500 hover:text-zinc-300">
            Back to login
          </Link>
        </div>
      </div>
