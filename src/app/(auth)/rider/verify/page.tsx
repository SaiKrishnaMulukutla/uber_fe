...
import { riderVerify, riderRegister } from '@/lib/api'
...
function VerifyForm() {
  ...
  const onSubmit = async () => {
    ...
    setLoading(true)
    try {
      const res = await riderVerify(email, code)
  ...
  const resend = async () => {
    try {
      // We don't have the original registration data here, so just show an info message
      toast.info('Please go back and register again to resend OTP')
    } catch {
        ...
        <CardContent className="space-y-6">
          ...
          <div className="text-center text-sm text-zinc-400">
            {countdown > 0 ? (
              ...
              <button onClick={resend} className="text-yellow-400 hover:underline">
                Resend OTP
              </button>
            )}
          </div>
