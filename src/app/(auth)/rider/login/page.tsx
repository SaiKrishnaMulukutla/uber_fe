...
export default function RiderLoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true)
    try {
      const res = await riderLogin(data.email, data.password)
      const { access_token, refresh_token, user } = res.data
      setRiderAuth(access_token, user!)
      await fetch('/api/auth/set-cookie', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ refresh_token, role: 'rider' }),
      })
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-black to-zinc-900 px-4">
      <Card className="w-full max-w-sm border-zinc-800 bg-zinc-950 text-white">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-zinc-400">Login to your RideGo account</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-4 text-center text-sm text-zinc-400">
            ...
            <Link href="/rider/register" className="text-yellow-400 hover:underline">Register</Link>
          </div>
          <div className="mt-2 text-center text-sm text-zinc-400">
            <Link href="/rider/forgot-password" className="text-zinc-500 hover:text-zinc-300">Forgot password?</Link>
          </div>
          <div className="mt-6 border-t border-zinc-800 pt-4 text-center text-sm text-zinc-500">
            Are you a driver?{' '}
            <Link href="/driver/login" className="text-yellow-400 hover:underline">Driver login</Link>
          </div>
        </CardContent>
      </Card>
    </div>
